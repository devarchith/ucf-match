...
export default async function MatchRevealPage() {
  ...
  try {
    const weekStatus = await getCurrentWeekStatus(session.userId);
    ...
    const matchedWithoutPreview =
      !preview && participationStatus === ParticipationStatus.MATCHED;
            ...
            preview ? (
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <p
                    id="match-response-unavailable"
                    className="text-xs text-muted-foreground"
                    role="note"
                  >
                    Accepting or passing a match is not available yet — there is no API for match
                    responses in this build.
                  </p>
                  <ResponsiveActionRow aria-describedby="match-response-unavailable">
                    <Button
                      type="button"
                      disabled
                      aria-disabled="true"
                      title="Not available until match response is supported by the backend."
                    >
                      Accept intro
                    </Button>
                    <Button
