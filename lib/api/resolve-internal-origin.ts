/**
 * Pure resolution for server→/api base URL (no `server-only`; unit-tested).
 * Async `next/headers` wrapper lives in `origin.ts`.
 */
 ...
 */
export function isDevLoopbackHostHeader(hostHeader: string): boolean {
  const hn = hostnameFromHostHeader(hostHeader);
  return hn === "localhost" || hn === "127.0.0.1" || hn === "::1";
}

/**
 * Resolve base URL from `process.env`-like map and request `Headers` (tests pass a plain `Headers`).
 */
export function resolveInternalApiOriginSync(
